...
q1_3_summary = q1_3 |>
  ...
  summarise(
    ...
    across(
      ...
      .fns = list(Mean = mean, Median = median, SD = sd, SE = se),
      .names = '{.col} {.fn}',
      na.rm = TRUE
    ),
...
#       `Estimate SD`, `Lower SD`, `Upper SD`, n
#       ) |>
#     mutate(
#       across(
...
ggplot(q2_long_clean, 
  ...
  theme(axis.title.x = element_blan)
...
region_count = demographics %>%
  ...
  arrange(desc(Category), desc(Count), Region) %>%
  mutate(Region = factor(Region, levels = c(unique(.$Region)[which(unique(.$Region) != 'Not Provided')], 'Not Provided')))
