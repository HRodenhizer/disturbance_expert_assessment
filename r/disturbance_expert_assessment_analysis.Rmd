...
```{r}
q5 = read_excel('data/Q5_not_done_optional.xlsx') |>
  rowwise() |>
  mutate(
...
q5_fp = read_excel('data/Q5_not_done_optional.xlsx', sheet = 2)
...
# q5_fp_summary_filter = q5_fp_summary |>
#   filter(!is.na(relationship) & from != 'CO2 Emissions' & to != 'CO2 Emissions')
# 
# q5_filter_indices = semi_join(
#   q5_fp_summary |>
#     mutate(idx = row_number()),
#   q5_fp_summary_filter,
#   by = colnames(q5_fp_summary_filter)
# ) |>
#   pull(idx)
```
...
#   id = 1:length(nodes),
#   name = nodes,
#   x = c(0, 2.5, 3.7, 3.5, 5.7, 8, 1.1, 2, 5.2, 2, 3.8, 4, 1.5, 0.7, 3.07, 5.3, 5.75, 3.6),
#   y = c(4, 4, 6.5, 4.9, 4, 4, 5.3, 5.9, 2.5, 3.3, 2.7, 2, 2.5, 6, 5.63, 5.3, 6, 3.5),
#   style = c('filled', rep('rounded', 4), 'filled', rep('rounded', 12)),
#   color = 'gray50',
#   fillcolor = c('gray70', rep('white', 4), 'gray70', rep('white', 12)),
#   shape = c('rectangle', rep('rectangle', 4), 'rectangle', rep('rectangle', 12)),
...
# 
# nodes_fp_filter = nodes_fp |>
#   filter(!name %in% c('Precipitation', 'Short-Lived Climate Forcers'))
# 
# edges_fp = tibble(
#   from = q5_fp_summary$from,
#   to = q5_fp_summary$to,
#   label = q5_fp_summary$label,
#   fontsize = 10,
#   # color = gray(((q5_fp_summary$count/(max(q5_fp_summary$count)*2)+0.25)*-1)+1),
#   # color = 'gray50',
#   color = case_when(q5_fp_summary$relationship == '-' ~ '#6699CC',
...
#   filter(style != 'dotted')
# 
# ggflowchart(edges_fp_filter, node_data = nodes_fp_filter, layout = 'custom')
```
