...
q5_fp_summary = q5_fp |>
  ...
  ranking = case_when(is.na(ranking) ~ 0,
                             TRUE ~ ranking)) |>
  summarise(ranking = mean(ranking)*n()/43, # normalizing for the number of times that path showed up in peoples' diagrams
            relationship = case_when(
              all(relationship == '-', na.rm = TRUE) ~ '-',
              all(relationship == '+', na.rm = TRUE) ~ '+',
              TRUE ~ '+/-'
            ),
            causation = mode(causation),
            count = n(),
            .by = c('from', 'to'))
```
...
nodes_fp = create_node_df(
  n = length(unique(c(q5_fp_summary$from, q5_fp_summary$to))),
  label = unique(c(q5_fp_summary$from, q5_fp_summary$to)),
  x = c(0, 2.5, 3, 3, 5.7, 8, 1, 2, 5.5, 2, 4, 4, 1.5, 1, 4.9, 6.2, 5.5, 3.6),
  y = c(4, 4, 6.2, 5, 4, 4, 5.3, 6, 2.5, 3.3, 2.7, 2, 2.5, 6.5, 5.6, 5, 6.5, 3.5),
  style = c('filled', rep('rounded', 4), 'filled', rep('rounded', 12)),
  color = 'gray20',
  fillcolor = c('gray30', rep('white', 4), 'gray30', rep('white', 12)),
...
edges_fp = create_edge_df(
  ...
  to = q5_fp_summary|>
    ...
    left_join(nodes_fp, by = 'label') |>
    pull(id),
  penwidth = q5_fp_summary$count,
  label = paste0(
    q5_fp_summary$relationship, 
    ' (', 
    round(q5_fp_summary$ranking, 1), 
    ')'
    )
)
edges_fp
...
fp = create_graph()
fp_1 = fp %>%
  add_nodes_from_table(table = nodes_fp,
                       label_col = label)
fp_2 = fp_1 %>%
  add_edges_from_table(table = edges_fp,
                       from_col = from,
                       to_col = to,
