...
q5_fp_summary = q5_fp |>
  ...
  summarise(
    ranking = case_when(
      all(ranking == 0) ~ NA_real_,
      TRUE ~ mean(ranking)
    ), # normalizing for the number of times that path showed up in peoples' diagrams
  ...
  mutate(
    label = case_when(
      is.na(ranking) ~ relationship,
      TRUE ~ paste0(relationship, 
                    ' (', 
                    round(ranking, 1), 
                    ')')
    )
...
nodes_fp = create_node_df(
  ...
  color = 'gray50',
  fillcolor = c('gray50', rep('white', 4), 'gray50', rep('white', 12)),
